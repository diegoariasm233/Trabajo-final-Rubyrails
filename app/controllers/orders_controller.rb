class OrdersController < ApplicationController
  ...
  def index
    ...
    if show_all
      ...
      @orders = Order.where(status: "Pendiente de aprobación del administrador")
    else
  ...
  def updated
    ...
    if params[:status] == "approve"
      ...
      @order.save!
      if @order.save
        redirect_to orders_path(:show_restricted => 'is_restricted'), notice: 'Solicitud de artículo restringido aprobado'
      else
        redirect_to orders_path(:show_restricted => 'is_restricted'), notice: 'Error al crear el retorno'
      end
    else
      @order.status = "Solicitud rechazada - Reembolsada"
      @order.save!
      if @order.save
        redirect_to orders_path(:show_restricted => 'is_restricted'), notice: 'Solicitud de artículo restringido rechazado'
      else
        redirect_to orders_path(:show_restricted => 'is_restricted'), notice: 'Error al crear el retorno'
      end
    end
